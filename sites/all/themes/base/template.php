...
function base_preprocess_node(&$vars){
	...
	if($vars['type'] == 'image_gallery'){
		...
		$vars['photographer'] = $vars['field_photographer'][0]['taxonomy_term']->name;
   	$vars['client'] = $vars['field_client'][0]['taxonomy_term']->name;
		...
		if($vars['view_mode'] == 'teaser'){
			...
			for($i = 0; $i < count($vars['field_other_images']); $i++){
		     	$images[] = render_image($vars['field_other_images'][$i], 'gallery_large');
		   }
